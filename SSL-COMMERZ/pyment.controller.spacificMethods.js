// Only for spacific payment methods
...
const order = async (req, res) => {
  try {
    ...
    formData.append(
      "success_url",
      `${process.env.SERVER_BASE_URL}/payment-success?q=success&tran_id=${tran_id}`
    );
    formData.append(
      "fail_url",
      `${process.env.SERVER_BASE_URL}/payment-success?q=fail`
    );
    formData.append(
      "cancel_url",
      `${process.env.SERVER_BASE_URL}/payment-success?q=cancel`
    );
    ...
    if (SSLResJson.status === "SUCCESS") {
      ...
      return res
        .status(200)
        .json({ status: true, url: [masterCard, bkash, nagad] });
    } else {
      return res
        .status(404)
